...
def generate_load_data(signal_data_path,which_line,which_letter,batch_size):
    ...
    while True:
        ...
        train_y0_batch = np.zeros((batch_size, 16))

        train_y1_batch = np.zeros((batch_size, 16))

        train_y2_batch = np.zeros((batch_size, 16))

        train_y3_batch = np.zeros((batch_size, 16))
        ...
        for indx,signal in enumerate(signal_list):
            ...
            wav = np.delete(wav, 35000)
            wav = wav.astype(np.float32)
            key_label_lb0 = to_categorical(read_filename_getLabel(signal,which_line,which_letter), 16)
            key_label_lb0 = np.expand_dims(key_label_lb0,axis=0)

            key_label_lb1 = to_categorical(read_filename_getLabel(signal,which_line,which_letter + 1), 16)
            key_label_lb1 = np.expand_dims(key_label_lb1,axis=0)

            key_label_lb2 = to_categorical(read_filename_getLabel(signal,which_line,which_letter + 2), 16)
            key_label_lb2 = np.expand_dims(key_label_lb2,axis=0)

            key_label_lb3 = to_categorical(read_filename_getLabel(signal,which_line,which_letter + 3), 16)
            key_label_lb3 = np.expand_dims(key_label_lb3,axis=0)
            ...
            train_y0_batch[indx_to_count] = key_label_lb0

            train_y1_batch[indx_to_count] = key_label_lb1

            train_y2_batch[indx_to_count] = key_label_lb2

            train_y3_batch[indx_to_count] = key_label_lb3


            if indx_to_count == batch_size-1:
                # print(train_x_batch.shape)
                # print(train_y_batch.shape)
                yield ({'input_1': train_x_batch}, {'output_tensor0': train_y0_batch,'output_tensor1': train_y1_batch,
                                                    'output_tensor2': train_y2_batch,'output_tensor3': train_y3_batch})
                train_x_batch = np.zeros((batch_size, 35000, 1))
...
if __name__=='__main__':
    ...
    g = generate_load_data_pool(signal_data_path='/data/wuchenxi/new_simeck_data/signal13533train_val_test/test',
